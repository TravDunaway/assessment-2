...
let greetUser = (friendName) => {
    return `Welcome back ${friendName}`
}
// console.log(greetUser('friendooo'))
...
function canWeDeliver(zipCode){
    for (let i = 0; i < deliveryAreaZipCodes.length; i++){
        if (zipCode === deliveryAreaZipCodes[i]){return "You are in our delivery area!"} else {return "you are not in our delivery zone"
...
function canWeDeliverTwo(zipCode){
    if(deliveryAreaZipCodes.includes(zipCode) === true){return "yo, you still be in our delivery area"} else {return "yo, ya aint in da deliverable area!"}
